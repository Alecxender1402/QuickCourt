...
const FlexibleBookingForm: React.FC<FlexibleBookingFormProps> = ({ venue }) => {
  ...
  const checkAvailability = async () => {
    ...
    // Function to check if booking time is within venue operating hours
  const isWithinOperatingHours = (): boolean => {
    if (!venue.workingHours || venue.workingHours.length === 0) {
      // If no working hours defined, allow booking (backward compatibility)
      return true;
    }
    ...
    const dayWorkingHours = venue.workingHours.find(wh => wh.dayOfWeek === dayOfWeek && wh.isActive);
    
    if (!dayWorkingHours) {
      toast({
        title: "Venue Closed",
        description: "The venue is closed on this day",
        variant: "destructive"
      });
      return false;
    }
    ...
    const venueOpenMinutes = timeToMinutes(dayWorkingHours.startTime);
    const venueCloseMinutes = timeToMinutes(dayWorkingHours.endTime);
    ...
    if (bookingStartMinutes < venueOpenMinutes) {
      const venueOpenTime = dayWorkingHours.startTime.substring(0, 5); // Remove seconds
      toast({
        title: "Booking Outside Operating Hours",
        description: `The venue opens at ${venueOpenTime}. Please select a start time at or after ${venueOpenTime}`,
        variant: "destructive"
      });
      return false;
    }
    ...
    if (bookingEndMinutes > venueCloseMinutes) {
      const venueCloseTime = dayWorkingHours.endTime.substring(0, 5); // Remove seconds
      toast({
        title: "Booking Outside Operating Hours",
        description: `The venue closes at ${venueCloseTime}. Please select an end time at or before ${venueCloseTime}`,
        variant: "destructive"
      });
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Calculate the amount and show payment modal
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            {submitting ? 'Creating Booking...' : 
             availability?.isAvailable ? `Book Court - ₹${availability.totalAmount?.toFixed(2)}` : 'Book Court'}
          </Button>
        </form>
      </CardContent>
